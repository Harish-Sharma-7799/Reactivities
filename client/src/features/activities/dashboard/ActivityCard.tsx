        ...
        <Card elevation={3} sx={{ borderRadius: 3 }}>
            <Box display='flex' alignItems='center' justifyContent='space-between'>
                <CardHeader avatar={<Avatar sx={{ height: 80, width: 80 }} />}
                    title={<span style={{ fontWeight: 'bold', fontSize: 20 }}>{activity.title}</span>}
                    subheader={<>
            ...
            <CardContent sx={{ pb: 2 }}>
                ...
                <Button sx={{ display: 'flex', justifySelf: 'self-end', borderRadius: 3 }} size="medium" variant="contained" component={Link} to={`/activities/${activity.id}`} onClick={() => { }}>
                    View
                </Button>
